function [VelocityDiffJWC,oldData,oldDataJWC] = logAllData(d,numberOfPeople,person,oldData,oldDataJWC,JDI,JII,JTS,JWC,PDI,PII,PWC,SD,firstLoop)
        ...
        if (numberOfPeople > 1)
            ...
            for i = 1:numberOfPeople
                ...
                for j = (i+1):numberOfPeople
                    ...
                    filename = sprintf('%d_%d_%IndivDist.csv',d.AbsTime(2),d.AbsTime(3),d.AbsTime(1));
            ...
            fclose(fid)
            fid = fopen('test.csv', 'w') ;
            filename = sprintf('%d_%d_%dAllDist.csv',d.AbsTime(2),d.AbsTime(3),d.AbsTime(1));
        ...
        if (firstLoop == 0)
            ...
            fid = fopen('test.csv', 'w') ;
            filename = sprintf('%d_%d_%d_V.csv',d.AbsTime(2),d.AbsTime(3),d.AbsTime(1));
           % dlmwrite(fullfile('C:\Users\aweber13\Desktop',filename),dataLine,'-append','delimiter',',')
            ...
            fid = fopen('test.csv', 'w') ;
            filename = sprintf('%d_%d_%d_V_JWC.csv',d.AbsTime(2),d.AbsTime(3),d.AbsTime(1));
           % dlmwrite(fullfile('C:\Users\aweber13\Desktop',filename),dataLine,'-append','delimiter',',')
