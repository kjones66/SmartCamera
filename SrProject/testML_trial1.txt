...
while ishandle(himg)
    ...
    if sum(depthMetaData.IsSkeletonTracked) > 0
       ...
       currentData = [[depthMetaData.FrameNumber;zeros19],[joints]];
       sprintf(defineFile);
       dlmwrite(defineFile,currentData,'-append','delimiter',',')
       fclose(fid)  
       hold on;
       ...
       hold off;
       trainedModel.RequiredVariables = [skeletonJoints(1,:,1)];
       p = trainedModel.predictFcn
%        prog = trainedModel;
%        position = jointsLive2(prog,currentData);
    end
end
